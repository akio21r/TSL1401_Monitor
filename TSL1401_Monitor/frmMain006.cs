...
namespace InvertedPendulumControl
{
	public partial class frmMain : Form
	{
		...
		int			rxpw = 0, rxpr = 0;				// 受信バッファの読み書きポインタ
			...
			while(ThreadContinue){
				if(serialPort1.IsOpen){
						...
						if(recv_mode == -1){				// レスポンス受信モード
							switch(recv_byte){
								...
								case 0xff:					// ログパケットの開始コード
									recv_mode = 0;
									rxbuf[rxpw++] = (byte)recv_byte;
									if(rxpw >= BUFFER_SIZE) rxpw = 0;
									break;
							}
						}
						else{								//ログ受信モード
							rxbuf[rxpw++] = (byte)recv_byte;	//満杯のときは上書きしていく
							if(rxpw >= BUFFER_SIZE) rxpw = 0;
							if(rxpw == rxpr){					//rxprに追いついたらrxpr++;
								rxpr++;
								if(rxpr >= BUFFER_SIZE) rxpr = 0;
							}
							...
							if( rxbuf[start] == 0xff ){
								//終了コードの位置を計算
								if(start + 1 < BUFFER_SIZE) end = start + rxbuf[start+1];
								else                        end = start + rxbuf[0];
								if(end >= BUFFER_SIZE) end -= BUFFER_SIZE;

								//終了コードの位置に 0xfe があったら、
								if( rxbuf[ end ] == 0xfe ){
					...
					if(logOK){			//buf[1]から読み取っていく（buf[0]はバイト数）
						...
						recv_data_n = (sbyte)rxbuf[0] / 2;

						for(i = 0; i < recv_data_n; i++){
							recv_data[i] = (sbyte)rxbuf[ i*2 + 1 ];
							recv_data[i] <<= 8;
							recv_data[i] |= rxbuf[ i*2 + 2 ];
						}
